        ...
        if(mission_state==APPROACH2)
          {
              if(scanornot == true)
              {
                  pose.pose.position.x = arucowp_x + distance_uavugv*(cos(angle_xy))*(1.5/frequency);    // 1.5 no ground effect
                  pose.pose.position.y = arucowp_y + distance_uavugv*(sin(angle_xy))*(speed/frequency);
                  pose.pose.position.z = arucowp_z - 0.3;        // fixed 0.35 m or arucowp_z-0.3 m

                  cout << "almost arrived........" << endl;
                  cout << arucocp_z << endl;
                  cout << arucowp_z << endl;
                  if(arucocp_z< 0.25 && arucowp_z <=0.26)      //wp 0.4
                  {
                      mission_state = LAND;
                      cout << "landing" << endl;
                      cout << arucocp_z << endl;
